...
void populate_model(Model& model)
{
    ...
    auto brickpi =
        model.add("PythonWorkload", "brickpi3_interface", 100.0, {{"script_name", "brickpi"}, {"class_name", "BrickPi3Interface"}});

    auto remote =
        model.add("PythonWorkload", "remote_control", 30.0, {{"script_name", "remote"}, {"class_name", "RemoteControlInterface"}});
...
int main()
{
    ...
    while (!g_exit_requested) std::this_thread::sleep_for(std::chrono::milliseconds(100));
