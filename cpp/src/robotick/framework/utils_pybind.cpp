...
namespace robotick {
	...
	template <size_t N>
	inline bool is_fixed_string_type(const std::type_index &t)
	{
		return t == typeid(FixedString<N>);
	}

	void marshal_struct_to_dict(void *struct_ptr,
								const StructRegistryEntry &info,
								py::dict &py_out)
	{
		for (const auto &field : info.fields) {
			void *field_ptr = static_cast<uint8_t *>(struct_ptr) + field.offset;

			if (dispatch_fixed_string(
					field.type, [&, field_ptr](auto T) -> bool {
						using FS = typename decltype(T)::type;
						py_out[field.name.c_str()] =
							static_cast<FS *>(field_ptr)->c_str();
						return true;
					})) {
				// handled by lambda
				...
				std::cerr
					<< "[marshal_struct_to_dict] Unsupported field type for "
					<< field.name << "\n";
			}
		}
	}

	void marshal_dict_to_struct(const py::dict &py_in,
								const StructRegistryEntry &info,
								void *struct_ptr)
	{
		for (const auto &field : info.fields) {
			...
			try {
				...
				std::cerr << "[marshal_dict_to_struct] Error converting field '"
						  << field.name << "': " << e.what() << "\n";
			}
		}
	}
