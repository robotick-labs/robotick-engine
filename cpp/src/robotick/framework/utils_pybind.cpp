    ...
    void marshal_dict_to_struct(const py::dict &py_in, const StructRegistryEntry &info, void *struct_ptr)
    {
        for (const auto &field : info.fields)
        {
            ...
            try
            {
                ...
                {
                    std::cerr << "[marshal_dict_to_struct] Unsupported field type for '" << field.name << "'\n";
                }
            }
            catch (const py::error_already_set &e)
            {
                std::cerr << "[marshal_dict_to_struct] Error converting field '" << field.name
                          << "': " << e.what() << "\n";
            }
        }
    }
