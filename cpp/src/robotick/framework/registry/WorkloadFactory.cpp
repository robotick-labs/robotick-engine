...
namespace robotick
{
    ...
    WorkloadHandle WorkloadFactory::add(const std::string& type_name, const std::string& name, const double tick_rate_hz,
                                        const std::map<std::string, std::any>& config)
    {
        if (m_finalised) throw std::runtime_error("Factory already finalised");
        const auto* entry = get_workload_registry_entry(type_name);
        if (!entry) throw std::runtime_error("Unknown workload type: " + type_name);
        m_pending_instances.push_back({entry, name, tick_rate_hz, config});
        return {static_cast<uint32_t>(m_pending_instances.size() - 1)};
    }
    ...
    {
        if (m_finalised) throw std::runtime_error("Already finalised");

        // Size and align
        size_t              offset = 0;
        std::vector<size_t> offsets;
        for (const auto& p : m_pending_instances)
        {
        ...
        for (size_t i = 0; i < m_pending_instances.size(); ++i)
        {
            const auto& p = m_pending_instances[i];

            // Check for duplicate names before adding
            bool name_exists = std::any_of(m_instances.begin(), m_instances.end(),
                                           [&](const WorkloadInstance& existing) { return existing.unique_name == p.name; });

            if (name_exists)
            {
                std::cerr << "Error: Duplicate unique_name '" << p.name << "' — skipping additional workload." << std::endl;
            ...
            if (p.type->config_struct)
                apply_struct_fields(static_cast<uint8_t*>(ptr) + p.type->config_offset, *p.type->config_struct, p.config);
