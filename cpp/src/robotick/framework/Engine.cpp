	...
	void Engine::load(const Model& model)
	{
		...
		for (size_t i = 0; i < workload_seeds.size(); ++i)
		{
			...
			if (type->config_struct)
				apply_struct_fields(static_cast<uint8_t*>(ptr) + type->config_offset, *type->config_struct,
									workload_seed.config);
			...
			m_impl->instances.push_back({ptr, type, workload_seed.name, workload_seed.tick_rate_hz});
		}
	}
