...
namespace robotick
{
	...
	Engine::Engine() : state(new Engine::State())
	{
	}
	...
	void Engine::load(const Model0& model_in)
	{
		if (state->model.get_root().is_valid())
		{
			ROBOTICK_FATAL_EXIT("Engine has already been loaded");
		}

		if (!model_in.get_root().is_valid())
		{
			ROBOTICK_FATAL_EXIT("Model0 has no root workload");
		}

		// store our model
		state->model = model_in;
		const Model0& model = state->model;

		// compute how much storage we will need for our workloads:
		...
		for (const auto& workload_seed : workload_seeds)
		{
			...
			if (!type)
			{
				ROBOTICK_FATAL_EXIT("Unknown workload type: %s", workload_seed.type.c_str());
			}
		...
		// allocate storage for our workloads and any blackboards
		...
		{
			const auto& seed = workload_seeds[i];
	...
	{
		const WorkloadHandle root_handle = state->model.get_root();
	...
	{
		for (const WorkloadInstanceInfo& instance : state->instances)
		{
			if (std::strcmp(instance.unique_name.c_str(), unique_name) == 0)
			{
				return &instance;
			}
		}
	...
	void Engine::setup_remote_engine_senders(const Model0& model)
	{
