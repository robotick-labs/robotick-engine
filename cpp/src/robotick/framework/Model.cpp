...
namespace robotick
{
	void Model0::connect_remote(const std::string& source_field_path, const std::string& dest_field_path)
	{
	...
	void Model0::connect(const std::string& source_field_path, const std::string& dest_field_path)
	{
		...
		if (root_workload.is_valid())
			ROBOTICK_FATAL_EXIT("Cannot add connections after root has been set. Model0 root must be set last.");

		data_connection_seeds.push_back({source_field_path, dest_field_path});
	}

	void Model0::finalize()
	{
		ROBOTICK_ASSERT(root_workload.is_valid() && "Model0 root must be set before validation");
	...
	WorkloadHandle Model0::add(const std::string& type, const std::string& name, double tick_rate_hz, const std::map<std::string, std::string>& config)
	{
		if (root_workload.is_valid())
			ROBOTICK_FATAL_EXIT("Cannot add workloads after root has been set. Model0 root must be set last.");
	...
	WorkloadHandle Model0::add(const std::string& type, const std::string& name, const std::vector<WorkloadHandle>& children, double tick_rate_hz,
	...
	void Model0::add_remote_model(const Model0& remote_model, const std::string& model_name, const std::string& comms_channel)
	{
		if (model_name.empty())
		{
	...
	void Model0::set_root(WorkloadHandle handle, const bool auto_finalize)
	{
