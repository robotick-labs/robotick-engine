...
namespace robotick
{
    ...
    WorkloadHandle WorkloadFactory::add_by_type(const std::string &type_name, const std::string &name, const std::map<std::string, std::any> &config)
    {
        if (m_finalised)
    ...
    void WorkloadFactory::finalise()
    {
        ...
        for (size_t i = 0; i < m_pending.size(); ++i)
        {
            ...
            if (p.type->config_struct)
                apply_struct_fields(static_cast<uint8_t *>(ptr) + p.type->config_struct_offset, *p.type->config_struct, p.config);
