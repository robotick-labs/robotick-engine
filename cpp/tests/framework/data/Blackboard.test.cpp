...
namespace
{
	std::pair<WorkloadsBuffer, Blackboard*> make_buffer_and_embedded_blackboard(const std::vector<BlackboardFieldInfo>& schema)
	{
...
TEST_CASE("Unit|Framework|Data|Blackboard|Blackboard basic construction and memory layout", "[blackboard]")
{
	...
	auto [buffer, blackboard] = make_buffer_and_embedded_blackboard(schema);

	REQUIRE(blackboard->get_info()->schema.size() == 3);
	REQUIRE(blackboard->get_info()->total_datablock_size >= sizeof(int) + sizeof(double) + sizeof(FixedString64));

	REQUIRE(blackboard->get_info()->has_field("age"));
	REQUIRE(blackboard->get_info()->has_field("score"));
	REQUIRE(blackboard->get_info()->has_field("name"));
	REQUIRE_FALSE(blackboard->get_info()->has_field("missing"));
}
