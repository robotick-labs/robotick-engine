...
TEST_CASE("Unit/Framework/Data/Buffer")
{
	SECTION("RawBuffer supports basic access and bounds checking", "[buffer]")
	{
		RawBuffer buffer(32);
	...
	SECTION("RawBuffer update_mirror_from validates size and performs copy", "[buffer][mirror]")
	{
		RawBuffer a(16);
		RawBuffer b(16);
	...
	SECTION("RawBuffer prevents duplicate create_mirror_from", "[buffer][mirror][lifecycle]")
	{
		RawBuffer source(8);
		RawBuffer mirror;
	...
	SECTION("RawBuffer throws if update_mirror_from used before allocation", "[buffer][mirror][invalid]")
	{
		RawBuffer source(8);
	...
	SECTION("RawBuffer contains_object checks bounds", "[buffer][bounds]")
	{
		RawBuffer buffer(32);
	...
	SECTION("RawBuffer handles alignment correctly in as<T>()", "[buffer][align]")
	{
		struct Aligned
		{
			int a;
