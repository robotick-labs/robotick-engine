...
TEST_CASE("Unit|Framework|WorkloadFactory|DummyWorkload stores tick rate")
{
	...
	factory.add_by_type("DummyWorkload", "ticky", 25.0, {});
	factory.finalise();
...
TEST_CASE("Unit|Framework|WorkloadFactory|Duplicate name is skipped")
{
	register_dummy_type();
	WorkloadFactory factory;
	factory.add_by_type("DummyWorkload", "duplicate", 30.0, {});
	factory.add_by_type("DummyWorkload", "duplicate", 60.0, {}); // same name
	factory.finalise();
...
TEST_CASE("Unit|Framework|WorkloadFactory|Throws on finalise twice")
{
	register_dummy_type();
	WorkloadFactory factory;
	factory.add_by_type("DummyWorkload", "x", 1.0, {});
	factory.finalise();
...
TEST_CASE("Unit|Framework|WorkloadFactory|Throws on add after finalise")
{
	...
	REQUIRE_THROWS_WITH(factory.add_by_type("DummyWorkload", "y", 2.0, {}), "Factory already finalised");
}

TEST_CASE("Unit|Framework|WorkloadFactory|Throws on unknown type")
{
	WorkloadFactory factory;
	REQUIRE_THROWS_WITH(factory.add_by_type("UnknownType", "x", 1.0, {}), "Unknown workload type: UnknownType");
}
...
TEST_CASE("Unit|Framework|WorkloadFactory|Applies config values to struct")
{
	register_dummy_type();
	WorkloadFactory factory;
	factory.add_by_type("DummyWorkload", "with_config", 1.0, {{"value", 123}});
	factory.finalise();
...
TEST_CASE("Unit|Framework|WorkloadFactory|get_type_name returns correct name")
{
	register_dummy_type();
	WorkloadFactory factory;
	auto handle = factory.add_by_type("DummyWorkload", "t1", 5.0, {});
	factory.finalise();
