...
namespace robotick
{
	...
	namespace detail
	{
		...
		{
			(void)buffer; // TODO - we definitely should be using this here!  WorkloadInstanceInfo shouldn't be storing an abs-pointer, but a
						  // buffer-offset

			uint8_t* struct_base = instance.ptr + struct_entry.offset;
		...
		{
			// TODO - the blackboard has its own internal way of doing this, but it uses knowledge "global singleton blackboard" - which needs to go
			// really...
			// AND remove the nasty const-cast!!
			return (void*)(buffer.raw_ptr() + blackboard.get_buffer_offset() + subfield.offset);
		}
	} // namespace detail

	struct WorkloadFieldsIterator
	{
		static inline void for_each_workload_field(const Engine& engine, const WorkloadsBuffer* workloads_override,
