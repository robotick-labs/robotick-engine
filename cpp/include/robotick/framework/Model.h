...
namespace robotick
{
	...
	class Model
	{
	  public:
		...
		WorkloadHandle add(const std::string& type, const std::string& name, const std::vector<WorkloadHandle>& children, double tick_rate_hz,
		...
		void set_root(WorkloadHandle handle) { m_root = handle; }
